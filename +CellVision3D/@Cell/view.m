...
%         imagesc(image);axis image;hold on; % JW
        imagesc(image(:,:,2));colormap gray; axis image; hold on;
    end
end
axis off;
...
for icell=1:length(cells)
    ...
    for ii=1:numcontours
        ...
        plot(bb{1}(:,1),bb{1}(:,2),'LineWidth',2,...
            'Color',colors(numparticlelabels+find(strcmp(labeltmp,...
            contourlabels)),:));
        hold on;
    end
    ...
    if ~isempty(cells(icell).particles)
        ...
        for i=1:size(cnt,1)
%             plot(cnt(i,1),cnt(i,2),'o','LineWidth',2,'Color',...
%                 colors(strcmp(labeltmp{i},particlelabels),:));hold on; %JW
            plot(cnt(i,1),cnt(i,2),'o','LineWidth',2,'Color','g');hold on;
        end
